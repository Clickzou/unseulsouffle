 ...
 *                            jc@clickzou.fr. JC seul : la cliente ne doit PAS
 *                            recevoir ces e-mails (consigne du 24/09/2026).
...
export async function GET(requete: Request) {
  ...
  const destinataires = (process.env.ALERTE_PUBLICATION_TO || "jc@clickzou.fr").split(",").map((s) => s.trim()).filter(Boolean);
